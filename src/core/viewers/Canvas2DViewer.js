...
export class Canvas2DViewer extends Abstract2DViewer {
  ...
  /**
   * Render the current Canvas2D view into an offscreen canvas at the given size.
   *
   * This implementation does not modify the on-screen canvas. It creates a
   * temporary off-screen Canvas2DViewer whose viewing component is sized to
   * the requested export dimensions so that camera/projection are computed
   * for the correct aspect ratio, then rasterizes that into the returned
   * canvas. Callers are responsible for any download/export logic.
   *
   * @param {number} width
   * @param {number} height
  ...
  async renderOffscreen(width, height, options = {}) {
    return await this._renderOffscreen('canvas2d', Canvas2DViewer, width, height, options);
  }
