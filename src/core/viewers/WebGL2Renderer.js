...
import { getLogger, Category, setModuleLevel, Level } from '../util/LoggingSystem.js';
...
import {
  renderInstancedGeometry as renderInstancedGeometryImpl
} from './webgl/WebGL2InstancedDraw.js';
...
export class WebGL2Renderer extends Abstract2DRenderer {
  ...
  /** @type {number} Transparency-derived alpha for polygon faces (1.0 = opaque, set in _beginPrimitiveGroup) */
  #polygonTransparencyAlpha = 1.0;
  ...
  visitAppearance(appearance) {
    ...
    if (this.#instancedRenderPending) {
      ...
      this.#instancedRenderPending = null;
      renderInstancedGeometryImpl(this.#buildInstancedDrawContext(), fundRegion, instanceTransforms, instanceColors, instanceCount);
    }
  }

  /**
   * Build the context object required by the extracted instanced-draw functions
   * in WebGL2InstancedDraw.js.  Bundles GL state, buffers, and renderer
   * call-backs so the instanced module does not need access to private fields.
   * @private
   * @returns {import('./webgl/WebGL2InstancedDraw.js').InstancedDrawContext}
   */
  #buildInstancedDrawContext() {
    return {
      gl: this.#gl,
      canvas: this.#canvas,
      programs: {
        unified: this.#unifiedProgram,
        fallback: this.#program,
      },
      ...
      buffers: {
        ...
        distance: this.#distanceBuffer,
        instRow0: this.#instRow0Buffer,
        instRow1: this.#instRow1Buffer,
        instRow2: this.#instRow2Buffer,
        instRow3: this.#instRow3Buffer,
        instColor: this.#instColorBuffer,
        sphereCenter: this.#sphereCenterBuffer,
        sphereColorInst: this.#sphereColorInstBuffer,
        tubeVertex: this.#tubeVertexBuffer,
        tubeIndex: this.#tubeIndexBuffer,
        tubeP0: this.#tubeP0Buffer,
        tubeP1: this.#tubeP1Buffer,
        tubeColorInst: this.#tubeColorInstBuffer,
      },
      tubeIndexCount: this.#tubeIndexCount,
      state: {
        currentColor: this.#currentColor,
        polygonTransparencyAlpha: this.#polygonTransparencyAlpha,
        debugGL: this.#debugGL,
      },
      getBooleanAttribute: (...args) => this.getBooleanAttribute?.(...args),
      getAppearanceAttribute: (...args) => this.getAppearanceAttribute(...args),
      beginPrimitiveGroup: (type) => this._beginPrimitiveGroup(type),
      endPrimitiveGroup: () => this._endPrimitiveGroup(),
      updateUniforms: (program, halfWidth, alpha, lighting, flipNormals) =>
        this.#updateUniforms(program, halfWidth, alpha, lighting, flipNormals),
      extractPoint: (v) => this._extractPoint(v),
      getCurrentTransformation: () => this.getCurrentTransformation(),
      getViewer: () => this._getViewer(),
      cacheGetVertexCoordsArray: (geom) => this._cacheGetVertexCoordsArray(geom),
      cacheGetEdgeIndicesArray: (geom) => this._cacheGetEdgeIndicesArray(geom),
      getOrCreateSphereMesh: (level) => this.#getOrCreateSphereMesh(level),
    };
  }
  ...
  _renderFaces(geometry) {
    ...
    const defaultColor = this.#currentColor;
    const tAlpha = this.#polygonTransparencyAlpha;
    const getFaceColor = (i) => {
      if (!faceColorsFlat) return defaultColor;
      const base = i * 4;
      return [faceColorsFlat[base], faceColorsFlat[base + 1], faceColorsFlat[base + 2], faceColorsFlat[base + 3] * tAlpha];
    };
  ...
  _beginPrimitiveGroup(type) {
    ...
    this.#polygonTransparencyAlpha = 1.0;
    try {
      const transparencyEnabled = this.getBooleanAttribute?.(
        CommonAttributes.TRANSPARENCY_ENABLED,
        DefaultRenderingHintsShader.TRANSPARENCY_ENABLED_DEFAULT
      ...
      if (transparencyEnabled && type === CommonAttributes.POLYGON) {
        const t = Number(this.getAppearanceAttribute(
         ...
         if (Number.isFinite(t) && t > 0) {
          ...
          this.#polygonTransparencyAlpha = alpha;
  ...
  #updateUniforms(program, lineHalfWidth = 0, pointSize = 1.0, lightingEnabled = false, flipNormals = false) {
    ...
    if (lightingEnabled && modelViewLoc !== null && normalMatLoc !== null) {
      ...
      if (o2w && w2c) {
        ...
        // Normal matrix: full 4x4 inverse-transpose of modelView.
        // For Euclidean normals (w=0) this reduces to the classical 3x3
        // inverse-transpose in the upper-left block; the 4th row/column
        // are carried along for future non-Euclidean metric support.
        const inv4 = Rn.inverse(null, mv);
        const n4 = Rn.transpose(null, inv4);
        logger.fine(Category.ALL, 'n4:', n4);
        gl.uniformMatrix4fv(normalMatLoc, true, new Float32Array(n4));
      }
    }
  ...
  #drawGeometry(vertices, colors, normals, indices, mode, distances = null, lineHalfWidth = 0, positionSize = 2, texCoords = null, texture2d = null) {
    ...
    if (normalLoc !== -1) {
      if (normals && normals.length > 0) {
        ...
        // Disable and set a safe default Euclidean normal (facing camera, w=0).
        gl.disableVertexAttribArray(normalLoc);
        gl.vertexAttrib4f(normalLoc, 0.0, 0.0, 1.0, 0.0);
        if (typeof gl.vertexAttribDivisor === 'function') {
          gl.vertexAttribDivisor(normalLoc, 0);
