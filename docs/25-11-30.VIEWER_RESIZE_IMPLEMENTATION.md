...
Added automatic resize functionality to `SVGViewer` to match the behavior of `Canvas2DViewer` and `WebGL2Viewer`. All three viewer implementations now automatically resize when their container element changes size.
...
viewerSwitch.selectViewer(1); // WebGL2  
