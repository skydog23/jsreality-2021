...
Implemented menu-based viewer switching with radio buttons in JSRViewer, eliminating the need for manual viewer management in HTML files. The feature provides a clean, reusable way to switch between Canvas2D, WebGL2, and SVG renderers through a standard menu interface.
...
**Features:**
- Creates JSRViewer with three viewers (Canvas2D, WebGL2, SVG)
...
jsrViewer.selectViewer(1);  // WebGL2
jsrViewer.selectViewer(2);  // SVG

// Switch viewer by name
jsrViewer.selectViewer('WebGL2');
...
### What to Verify
- ✓ Menu shows three radio buttons (Canvas2D, WebGL2, SVG)
...
jsrViewer.selectViewer(1);  // Switch to WebGL2 - menu should update
...
<div class="menu">
  ...
  <input type="radio" name="viewer" value="1"> WebGL2
